...
import type { ChurnTrendData } from "@/app/actions"
...
export default function ChurnTrendChart({ data = [] }: ChurnTrendChartProps) {
  ...
  const filterDataByDateRange = (data: ChurnTrendData[]) => {
  ...
  useEffect(() => {
    ...
    const rates = filteredData.map((item) => item.churn_rate_percent)
